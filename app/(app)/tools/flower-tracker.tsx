...
export default function FlowerTrackerScreen() {
  ...
  useEffect(() => {
    loadJSON<FlowerData>(KEYS.flowerTracker, {
      vendorName: 'Flower Vendor',
      pricePerDay: 0,
      deliveries: [],
      payments: [],
      ...
      setPriceInput(d.pricePerDay > 0 ? String(d.pricePerDay) : '');
    });
  }, []);

  const persist = useCallback((d: FlowerData) => {
    setData(d);
    saveJSON(KEYS.flowerTracker, d);
  }, []);
  ...
  const saveSettings = () => {
    persist({ ...data, vendorName: nameInput.trim() || 'Flower Vendor', pricePerDay: parseFloat(priceInput) || 0 });
  ...
  const totalCost = daysDelivered * data.pricePerDay;
    ...
    <ScreenShell
      ...
      rightAction={
        <View style={{ flexDirection: 'row', gap: 12 }}>
          ...
          <TouchableOpacity onPress={() => { setNameInput(data.vendorName); setPriceInput(data.pricePerDay > 0 ? String(data.pricePerDay) : ''); setShowSettings(true); }}>
    ...
    >
      ...
      <View style={[styles.summaryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        <View style={styles.summaryGrid}>
          ...
          <View style={styles.summaryItem}>
            <Text style={[styles.summaryVal, { color: ACCENT }]}>{totalCost.toFixed(0)}</Text>
      ...
      <Modal visible={showSettings} transparent animationType="fade" onRequestClose={() => setShowSettings(false)}>
        <View style={styles.modalBg}>
          <View style={[styles.modalCard, { backgroundColor: colors.bg }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>Flower Settings</Text>
            ...
            />
            <Text style={[styles.fieldLabel, { color: colors.textMuted }]}>Price per Day</Text>
            <TextInput
              style={[styles.modalInput, { backgroundColor: colors.inputBg, borderColor: colors.border, color: colors.text }]}
              value={priceInput} onChangeText={setPriceInput} placeholder="e.g. 50" placeholderTextColor={colors.textMuted} keyboardType="numeric"
            />
          ...
          <View style={[styles.modalCard, { backgroundColor: colors.bg }]}>
            ...
            {data.pricePerDay > 0 && (
              <View style={styles.quickRow}>
                {[data.pricePerDay * 30, data.pricePerDay * 15, 500, 1000].map(a => (
